...
public class JnrTestParallelRunnerTestJnrTest extends JnrTest { // NOSONAR
	...
	protected @Override void specify() {
		...
		test("should run in parallel", () -> {
			...
			JnrTestRunner runner = new JnrTestParallelRunner().add(new JnrTest("a test class with success") {
				@Override
			...
			var aggregatedResults = new JnrTestResultAggregator().aggregate(testRecorderWithElapsed);
